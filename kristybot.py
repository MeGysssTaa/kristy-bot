    ...
    try:
        if not chats.find_one({"chat_id": chat_id, "members": {"$eq": user_id}},
            ...
            chats.update_one({"chat_id": chat_id, "members.user_id": {"$ne": user_id}},
                             {"$push": {"members": {"user_id": user_id, "rank": 0, "all": 0}}})
    except:
                ...
                if command == "подключиться":
                    try:
                        ...
                        if not re.findall(r"(?<=\s)[a-zA-Zа-яА-ЯёЁ\d]+(?=\s|$)", event.object.message["text"]):
                            vk.messages.send(chat_id=event.chat_id,
                                             message="Вы не ввели группы, либо использовали запрешённые символы в названиях",
                                             random_id=int(vk_api.utils.get_random_id()))
                            continue
                        groups_find = list(
                            set(re.findall(r"(?<=\s)[a-zA-Zа-яА-ЯёЁ\d]+(?=\s|$)", event.object.message["text"])) - set(
                                ["all", "все", "online", "онлайн"]))
                        groups_error = list(
                            set(groups_find) - set(chats.distinct("groups.name", {"chat_id": event.chat_id})))
                        groups_on = list(set(groups_find) - set(groups_error))
                        groups_user = list(chats.aggregate([{"$unwind": "$groups"}, {"$match": {
                            "$and": [{"chat_id": event.chat_id},
                        ...
                        for group in groups_off:
                            chats.update_one({"chat_id": event.chat_id, "groups.name": group},
                                             {"$push": {"groups.$.members": event.object.message["from_id"]}})
                        ...
                        if not groups_off and not groups_error:
                            ...
                            vk.messages.send(chat_id=event.chat_id,
                                             message=name + "\nК сожалению, вы не создавали эти группы:\n🚫 " + '\n🚫 '.join(
                                                 groups_error), random_id=int(vk_api.utils.get_random_id()))
                    ...
                    try:
                        ...
                        for group in groups_on:
                            chats.update_one({"chat_id": event.chat_id, "groups.name": group},
                                             {"$pull": {"groups.$.members": event.object.message["from_id"]}})
                    ...
                    try:
                        if chats.find_one({"chat_id": event.chat_id}, {"_id": 0, "members": {
                            "$elemMatch": {"user_id": {"$eq": event.object.message["from_id"]}, "rank": {"$gt": 0}}}}):
                            if '>' not in event.object.message["text"]:
                                vk.messages.send(chat_id=event.chat_id, message="Не найден символ '>'",
                                                 random_id=int(vk_api.utils.get_random_id()))
                                continue
                            users_find = re.findall(r"\[id+(\d+)\|\W*\w+\]", event.object.message["text"].split('>')[0])
                            users_id_int = [int(user) for user in users_find]
                            if not users_find:
                                vk.messages.send(chat_id=event.chat_id, message="Пользователи не найдены",
                                                 random_id=int(vk_api.utils.get_random_id()))
                                continue
                            groups_find = list(set(re.findall(r"(?<=\s)[a-zA-Zа-яА-ЯёЁ\d]+(?=\s|$)",
                                                              event.object.message["text"].split('>')[1])) - set(
                                ["all", "все", "online", "онлайн"]))
                            if not groups_find:
                                vk.messages.send(chat_id=event.chat_id,
                                                 message="Вы не ввели группы, либо использовали запрешённые символы в названиях",
                                                 random_id=int(vk_api.utils.get_random_id()))
                                continue
                            groups_all = list(chats.aggregate([{"$unwind": "$groups"}, {"$match": {
                                "$and": [{"chat_id": event.chat_id}, {"groups.name": {"$in": groups_find}}]}}, {
                                                                   "$group": {"_id": "$chat_id",
                                                                              "groups": {"$push": "$groups.name"}}}]))[
                                0]["groups"]
                            if not groups_all:
                                vk.messages.send(chat_id=event.chat_id, message="Вы данный момент в группе нет групп",
                                                 random_id=int(vk_api.utils.get_random_id()))
                                continue

                            users_id_int = [int(user) for user in users_find]

                            members = list(chats.aggregate([{"$unwind": "$members"}, {"$match": {
                                "$and": [{"chat_id": event.chat_id}, {"members.user_id": {"$in": users_id_int}}]}}, {
                                                                "$group": {"_id": "$chat_id",
                                                                           "members": {"$push": "$members.user_id"}}}]))
                            ...
                            for user in users_ok:
                                ...
                                groups_all_user = list(chats.aggregate([{"$unwind": "$groups"}, {"$match": {
                                    "$and": [{"chat_id": event.chat_id}, {"groups.members": {"$eq": user}},
                                             {"groups.name": {"$in": groups_all}}]}}, {"$group": {"_id": "$chat_id",
                                                                                                  "groups": {
                                                                                                      "$push": "$groups.name"}}}]))
                                if groups_all_user:
                            ...
                            for user in first_names_dict.items():
                                message += "[id{0}|{1}]".format(str(user[0]), user[1]) + ' > ' + ', '.join(
                                    user_groups[user[0]]) + "\n"
                            ...
                            if message:
                                ...
                                vk.messages.send(chat_id=event.chat_id,
                                                 message="Данные пользователи уже состоят в этих группах",
                                                 random_id=int(vk_api.utils.get_random_id()))
                        else:
                            vk.messages.send(chat_id=event.chat_id, message="У вас нет прав админа или короля",
                                             random_id=int(vk_api.utils.get_random_id()))
                    except Exception as ex:
                        traceback.print_exc()
                        vk.messages.send(chat_id=event.chat_id, message="Что-то пошло не так(((",
                    ...
                    try:
                        if chats.find_one({"chat_id": event.chat_id}, {"_id": 0, "members": {
                            ...
                            for user in re.findall(r"\[id+(\d+)\|\W*\w+\]", event.object.message["text"].split('>')[0]):
                                user_groups.update({user: []})
                                for group in re.findall(r"(?<=\s)[a-zA-Zа-яА-ЯёЁ\d]+(?=\s|$)",
                                                        event.object.message["text"].split('>')[1]):
                                    if int(user) in usersall and group in groupsall and chats.find_one(
                                            {"chat_id": event.chat_id, "groups": {
                                                "$elemMatch": {"name": {"$eq": group}, "members": {"$eq": int(user)}}}},
                                            {"_id": 0, "groups.name.$": 1}):
                                        chats.update_one({"chat_id": event.chat_id, "groups.name": group},
                                                         {'$pull': {"groups.$.members": int(user)}})
                                        user_groups[user].append(group)
                                if not user_groups[user]:
                                    del user_groups[user]
                            message = "Отключила:\n"
                            ...
                            for user in user_groups:
                                message += "[id{0}|{1}]".format(str(user),
                                                                first_names_dict[str(user)]) + ' > ' + ', '.join(
                                    user_groups[user]) + "\n"
                            if message != "Отключила:\n":
                                ...
                                vk.messages.send(chat_id=event.chat_id,
                                                 message="Данные пользователи не состоят в этих группах",
                                                 random_id=int(vk_api.utils.get_random_id()))
                        else:
                            vk.messages.send(chat_id=event.chat_id, message="У вас нет прав админа или короля",
                                             random_id=int(vk_api.utils.get_random_id()))
                ...
                # Системные команды
                    ...
                    try:
                        if chats.find_one({"chat_id": event.chat_id, "members": {
                            "$elemMatch": {"user_id": {"$eq": event.object.message["from_id"]}, "rank": {"$gt": 0}}}},
                                          {"_id": 0, "members.user_id.$": 1}):
                            admins = []
                            users_error = []
                            users_find = re.findall(r"\[id+(\d+)\|\W*\w+\]", event.object.message["text"])
                            if not users_find:
                                vk.messages.send(chat_id=event.chat_id, message="Вы не указали пользователей",
                                                 random_id=int(vk_api.utils.get_random_id()))
                                continue
                            users_id_int = [int(user) for user in users_find]
                            ...
                            members = list(chats.aggregate([{"$unwind": "$members"}, {"$match": {
                                "$and": [{"chat_id": event.chat_id}, {"members.user_id": {"$in": users_id_int}},
                                         {"members.rank": {"$eq": 0}}]}}, {"$group": {"_id": "$chat_id", "members": {
                            ...
                            if members:
                                ...
                                for user in users:
                                    chats.update_one({"chat_id": event.chat_id, "members.user_id": int(user["id"])},
                                                     {"$set": {"members.$.rank": 1}})
                                    admins.append("[id{0}|{1}]".format(str(user["id"]), user["first_name"]))
                            else:
                                users_not = users_id_int

                            members_admin = list(chats.aggregate([{"$unwind": "$members"}, {"$match": {
                                "$and": [{"chat_id": event.chat_id}, {"members.user_id": {"$in": users_not}},
                                         {"members.rank": {"$gt": 0}}]}}, {"$group": {"_id": "$chat_id", "members": {
                                "$push": "$members.user_id"}}}]))
                            if members_admin:
                            ...
                            if admins and users_error:
                                vk.messages.send(chat_id=event.chat_id,
                                                 message="Поздравляем новых админов!!!: " + ', '.join(admins)
                                                     ...
                                                     users_error), random_id=int(vk_api.utils.get_random_id()))
                                ...
                                vk.messages.send(chat_id=event.chat_id,
                                                 message="Пользователей нет в базе данных. Попросите их написать сообщение в чат, либо используйте !download(с админ правами): \n@id" + " \n@id".join(
                                                     users_error), random_id=int(vk_api.utils.get_random_id()))
                            else:
                                vk.messages.send(chat_id=event.chat_id, message="Все уже в админах!!!",
                                                 random_id=int(vk_api.utils.get_random_id()))
                        else:
                            vk.messages.send(chat_id=event.chat_id, message="У вас нет прав админа или короля",
                                             random_id=int(vk_api.utils.get_random_id()))
                        ...
                        traceback.print_exc()
                        vk.messages.send(chat_id=event.chat_id, message=traceback.format_exc(),
                                         random_id=int(vk_api.utils.get_random_id()))
                elif command == "unadmin":
                    try:
                        admins = []
                        if chats.find_one({"chat_id": event.chat_id, "members": {
                            "$elemMatch": {"user_id": {"$eq": event.object.message["from_id"]}, "rank": {"$eq": 2}}}},
                                          {"_id": 0, "members.user_id.$": 1}):
                            users_id = re.findall(r"\[id+(\d+)\|\W*\w+\]", event.object.message["text"])
                            for user in users_id:
                                member = chats.find_one({"chat_id": event.chat_id}, {"_id": 0, "members": {
                                    "$elemMatch": {"user_id": {"$eq": int(user)}}}})
                                if member and member["members"][0]["rank"] == 1:
                                    first_name = vk.users.get(user_id=user)[0]["first_name"]  # оптимизировать
                                    chats.update_one({"chat_id": event.chat_id, "members.user_id": int(user)},
                                                     {"$set": {"members.$.rank": 0}})
                                    admins.append("[id{0}|{1}]".format(str(user), first_name))
                            if admins:
                                vk.messages.send(chat_id=event.chat_id,
                                                 message="Мы прощаемся с этими людьми в рядах админов: " + ', '.join(
                                                     admins), random_id=int(vk_api.utils.get_random_id()))
                            else:
                                vk.messages.send(chat_id=event.chat_id,
                                                 message="Похоже, все уже и так не в админах, либо их нет в беседе",
                                                 random_id=int(vk_api.utils.get_random_id()))
                        else:
                            vk.messages.send(chat_id=event.chat_id, message="Вы не король",
                                             random_id=int(vk_api.utils.get_random_id()))
                    except Exception as ex:
                        traceback.print_exc()
                        vk.messages.send(chat_id=event.chat_id, message="Что-то пошло не так(((",
                    ...
                    try:
                        if chats.find_one({"chat_id": event.chat_id, "members": {
                            ...
                            member = chats.find_one({"chat_id": event.chat_id, "members": {"$elemMatch": {
                                "$and": [{"user_id": {"$eq": int(user)}},
                                         {"user_id": {"$ne": event.object.message["from_id"]}}]}}},
                                                    {"_id": 0, "members.user_id.$": 1})
                            if member:
                                ...
                                chats.update_one({"chat_id": event.chat_id, "members.user_id": int(user)},
                                                 {"$set": {"members.$.rank": 2}})
                                chats.update_one(
                                    {"chat_id": event.chat_id, "members.user_id": int(event.object.message["from_id"])},
                                    {"$set": {"members.$.rank": 1}})
                                vk.messages.send(chat_id=event.chat_id,
                                                 message="ПРИВЕТСТВУЙТЕ НОВОГО КОРОЛЯ: " + "[id{0}|{1}]".format(
                                                     str(user), first_name),
                                                 random_id=int(vk_api.utils.get_random_id()))
                            else:
                                vk.messages.send(chat_id=event.chat_id,
                                                 message="Похоже его нет в беседе, либо вы хотели сделать себя НОВЫМ КОРОЛЁМ",
                                                 random_id=int(vk_api.utils.get_random_id()))
                        else:
                            vk.messages.send(chat_id=event.chat_id, message="Вы не король",
                                             random_id=int(vk_api.utils.get_random_id()))
                    ...
                    try:
                        king = list(chats.aggregate([{"$unwind": "$members"}, {
                            "$match": {"$and": [{"chat_id": event.chat_id}, {"members.rank": {"$eq": 2}}]}}, {
                                                         "$group": {"_id": "$chat_id",
                                                                    "king": {"$push": "$members.user_id"}}}]))[0][
                            "king"][0]
                        admins_find = list(chats.aggregate([{"$unwind": "$members"}, {
                            "$match": {"$and": [{"chat_id": event.chat_id}, {"members.rank": {"$eq": 1}}]}}, {
                                                                "$group": {"_id": "$chat_id",
                                                                           "admins": {"$push": "$members.user_id"}}}]))
                        ...
                        vk.messages.send(chat_id=event.chat_id, message=kingtext + admin_text,
                                         random_id=int(vk_api.utils.get_random_id()))
                    except Exception as ex:
                        traceback.print_exc()
                        vk.messages.send(chat_id=event.chat_id, message="Что-то пошло не так(((",
                    ...
                    if int(time.strftime("%W", time.gmtime(time.time() + 2 * 60 * 60))) % 2 == 0:
                        vk.messages.send(chat_id=event.chat_id, message="НИЖНЯЯ НЕДЕЛЯ",
                                         random_id=int(vk_api.utils.get_random_id()))
                    else:
                        vk.messages.send(chat_id=event.chat_id, message="ВЕРХНЯЯ НЕДЕЛЯ",
                                         random_id=int(vk_api.utils.get_random_id()))
                elif command == "бфу":
