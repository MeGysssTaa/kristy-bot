...
class TimetableData:
    ...
    # TODO 3: брать расписание из удалённого источника (github gist, например)
            ...
            if file.endswith(TIMETABLE_FILE_EXT):
                with open('timetables/' + file, 'r', encoding='UTF-8') as fstream:
                    try:
                        ...
                        try:
                            self._parse_timetable(owner_chat_id, timetable_yml)
                            self.logger.info('Загружен файл с расписанием беседы № %i', owner_chat_id)
                        except Exception as e:
                            self.logger.warning('Не удалось обработать файл с расписанием беседы № %i:', owner_chat_id)

                            if isinstance(e, SyntaxError):
                                self.logger.warning('%s', e)
                                self.kristy.send(owner_chat_id + 2E9,
                            ...
                            if owner_chat_id in self.timezones:
                                del self.timezones[owner_chat_id]

                            if owner_chat_id in self.class_ordinals:
                                del self.class_ordinals[owner_chat_id]

                            if owner_chat_id in self.classes:
                                del self.classes[owner_chat_id]
                    except ValueError:
                        self.logger.warning('Файл с расписанием %s назван некорректно '
                                            '(формат: "ЧИСЛЕННЫЙ_ID_БЕСЕДЫ.%s")', file, TIMETABLE_FILE_EXT)
                    except yaml.YAMLError:
                        self.logger.warning('Не удалось прочитать файл с расписанием беседы № %i:', owner_chat_id)
                        traceback.print_exc()
