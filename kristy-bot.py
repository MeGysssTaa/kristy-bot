            ...
            try:
                ...
                if event.text.split()[1] in data[event.chat_id]:
                    ...
                    if userdomain in data[event.chat_id][event.text.split()[1]]:
                        vk.messages.send(chat_id = event.chat_id, message = "Вы уже состоите в группе: " + event.text.split()[1], random_id = int(vk_api.utils.get_random_id()))
                    else:
                        data[event.chat_id][event.text.split()[1]].append(userdomain)
                        vk.messages.send(chat_id = event.chat_id, message = "Успешно добавила ВАС в группу: " + event.text.split()[1], random_id = int(vk_api.utils.get_random_id()))
                else:
                    vk.messages.send(chat_id = event.chat_id, message = "Такой группы нет((", random_id = int(vk_api.utils.get_random_id()))
            ...
            try:
                ...
                if event.text.split()[1] in data[event.chat_id]:
                    ...
                    if userdomain in data[event.chat_id][event.text.split()[1]]:
                        ...
                        vk.messages.send(chat_id = event.chat_id, message = "Успешно отключила ВАС от группы: " + event.text.split()[1], random_id = int(vk_api.utils.get_random_id()))
                    else:
                        vk.messages.send(chat_id = event.chat_id, message = "Вы не состоите в этой группе: " + event.text.split()[1], random_id = int(vk_api.utils.get_random_id()))
                else:
                    vk.messages.send(chat_id = event.chat_id, message = "Такой группы нет((", random_id = int(vk_api.utils.get_random_id()))
            except Exception as ex:
                print(Exception)
            ...
            try:
                ...
                if event.text.split()[1] in data[event.chat_id]:
                    ...
                    for user in data[event.chat_id][event.text.split()[1]]:
                        ping += "@" + user + " "
                    if ping:
                        vk.messages.send(chat_id = event.chat_id, message = "ПИНГУЮ: " + ping, random_id = int(vk_api.utils.get_random_id()))
                    else:
                        vk.messages.send(chat_id = event.chat_id, message = "Никого нет в этой группе : " + event.text.split()[1], random_id = int(vk_api.utils.get_random_id()))
                else:
                    vk.messages.send(chat_id = event.chat_id, message = "Такой группы нет((", random_id = int(vk_api.utils.get_random_id()))
            except Exception as ex:
                print(Exception)
