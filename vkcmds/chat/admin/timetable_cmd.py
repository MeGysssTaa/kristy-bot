...
class TimetableCommand(VKCommand):
    def __init__(self, kristy):
        VKCommand.__init__(self, kristy,
                           label='расписание',
                           desc='Позволяет управлять расписанием '
                                '(новая ссылка, вывод текущей, перезагрузка файла, отключение)',
                           usage='!расписание [ссылка на новый файл | "обновить" | "отключить"]',
    ...
    def execute(self, chat, peer, sender, args=None, attachments=None, fwd_messages=None):
        if args:
            ...
            elif new_url.lower() == 'отключить':
                # Отключаем расписание путём сброса ссылки на файл.
                self.kristy.db.set_timetable_url(chat, '')
                self.kristy.tt_data.load_timetable(chat, hide_errors=True)
                self.kristy.send(peer, '⛔ Отключила весь функционал расписания в беседе')
            else:
