...
class Status(Minigame):
    def __init__(self, kristy):
        Minigame.__init__(self, kristy,
                          label='статус',
                          rules='Вам будут показываться статусы участников беседы. '
                          ...
                          usage='!игра статус <число раундов от {0} до {1}>'.format(MIN_ROUNDS, MAX_ROUNDS),
                          min_args=1)
    ...
    def start_game(self, chat, peer, sender):
        ...
        answer, status = self.get(chat, peer)
        ...
        self.kristy.send(peer, "Игра началась. Чей же это статус? \n" + status)
    ...
    def check_game(self, chat, peer, sender, msg):
        ...
        if self.kristy.minigames[chat]['round'] == self.kristy.minigames[chat]['max_rounds']:
            ...
            response += "Игра продолжается. Следующий статус через 5 секунд."
            ...
            self.kristy.send(peer, "Чей же это статус? \n" + status)
    ...
    def get(self, chat, peer):
        ...
        for i in range(len(users_vk)):
            ...
            if random_users["status"]:
                return random_users["first_name"], str(random_users["status"])
        return None, None
